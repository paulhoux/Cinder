...
CScheduler::CScheduler(CCritSec& critSec) :
    m_nRefCount(1),
    m_critSec(critSec),
    m_pCB(NULL),
...
ULONG CScheduler::AddRef()
{
    return InterlockedIncrement(&m_nRefCount);
}

ULONG  CScheduler::Release()
{
...
HRESULT CScheduler::QueryInterface(REFIID iid, __RPC__deref_out _Result_nullonfailure_ void** ppv)
{
    if (!ppv)
    {
        return E_POINTER;
...
void CScheduler::SetFrameRate(const MFRatio& fps)
{
...
HRESULT CScheduler::StartScheduler(IMFClock* pClock)
{
    HRESULT hr = S_OK;
...
HRESULT CScheduler::StopScheduler(void)
{
    if (m_hWaitTimer != NULL)
    {
        CloseHandle(m_hWaitTimer);
...
HRESULT CScheduler::ProcessSample(IMFSample* pSample, LONG* plNextSleep)
{
    HRESULT hr = S_OK;
...
HRESULT CScheduler::StartProcessSample()
{
    HRESULT hr = S_OK;
...
//
//--------------------------------------------------------------------------
HRESULT CScheduler::OnTimer(__RPC__in_opt IMFAsyncResult *pResult)
{
    HRESULT hr = S_OK;
