#include "cinder/msw/dx11/MediaSink.h"
...
CCritSec CMediaSink::s_csStreamSinkAndScheduler;
...
/* static */ HRESULT CMediaSink::CreateInstance(_In_ REFIID iid, _COM_Outptr_ void** ppSink)
{
    if (ppSink == NULL)
    {
        return E_POINTER;
...
ULONG CMediaSink::AddRef(void)
{
    return InterlockedIncrement(&m_nRefCount);
}

HRESULT CMediaSink::QueryInterface(REFIID iid, __RPC__deref_out _Result_nullonfailure_ void** ppv)
{
    if (!ppv)
    {
        return E_POINTER;
...
HRESULT CMediaSink::AddStreamSink(DWORD dwStreamSinkIdentifier, __RPC__in_opt IMFMediaType* pMediaType, __RPC__deref_out_opt IMFStreamSink** ppStreamSink)
{
...
HRESULT CMediaSink::GetCharacteristics(__RPC__out DWORD* pdwCharacteristics)
{
...
HRESULT CMediaSink::GetPresentationClock(__RPC__deref_out_opt IMFPresentationClock** ppPresentationClock)
{
    CAutoLock lock(&m_csMediaSink);
...
HRESULT CMediaSink::GetStreamSinkById(DWORD dwStreamSinkIdentifier, __RPC__deref_out_opt IMFStreamSink** ppStreamSink)
{
    CAutoLock lock(&m_csMediaSink);
...
HRESULT CMediaSink::GetStreamSinkByIndex(DWORD dwIndex, __RPC__deref_out_opt IMFStreamSink** ppStreamSink)
{
    CAutoLock lock(&m_csMediaSink);
...
HRESULT CMediaSink::GetStreamSinkCount(__RPC__out DWORD* pcStreamSinkCount)
{
    CAutoLock lock(&m_csMediaSink);
...
HRESULT CMediaSink::RemoveStreamSink(DWORD dwStreamSinkIdentifier)
{
    return MF_E_STREAMSINKS_FIXED;
}
...
HRESULT CMediaSink::SetPresentationClock(__RPC__in_opt IMFPresentationClock* pPresentationClock)
{
    CAutoLock lock(&m_csMediaSink);
...
HRESULT CMediaSink::Shutdown(void)
{
    CAutoLock lock(&m_csMediaSink);
...
HRESULT CMediaSink::OnClockPause(
...
HRESULT CMediaSink::OnClockRestart(
    /* [in] */ MFTIME hnsSystemTime)
{
    CAutoLock lock(&m_csMediaSink);
...
HRESULT CMediaSink::OnClockSetRate(
    /* [in] */ MFTIME hnsSystemTime,
    /* [in] */ float flRate)
{
    if (m_pScheduler != NULL)
...
HRESULT CMediaSink::OnClockStart(
    /* [in] */ MFTIME hnsSystemTime,
...
HRESULT CMediaSink::OnClockStop(
    /* [in] */ MFTIME hnsSystemTime)
{
    CAutoLock lock(&m_csMediaSink);
...
HRESULT CMediaSink::GetService(__RPC__in REFGUID guidService, __RPC__in REFIID riid, __RPC__deref_out_opt LPVOID* ppvObject)
{
    HRESULT hr = S_OK;
...
STDMETHODIMP CMediaSink::GetFastestRate(
...
STDMETHODIMP CMediaSink::GetSlowestRate(
    MFRATE_DIRECTION eDirection,
    BOOL fThin,
    _Out_ float* pflRate
    )
...
STDMETHODIMP CMediaSink::IsRateSupported(BOOL fThin, float flRate, __RPC__inout_opt float* pflNearestSupportedRate)
{
    HRESULT hr = S_OK;
...
HRESULT CMediaSink::CheckShutdown(void) const
{
    if (m_IsShutdown)
    {
...
HRESULT CMediaSink::Initialize(void)
{
    HRESULT hr = S_OK;
