...
/* static */ HRESULT MediaSink::CreateInstance( _In_ REFIID iid, _COM_Outptr_ void** ppSink, DirectXVersion dxVersion )
{
	if( ppSink == NULL ) {
		return E_POINTER;
	}
	...
	*ppSink = NULL;
	...
	HRESULT hr = S_OK;
	MediaSink* pSink = new MediaSink( dxVersion ); // Created with ref count = 1.
...
MediaSink::MediaSink( DirectXVersion dxVersion )
	: STREAM_ID( 0 )
	, m_nRefCount( 1 )
...
HRESULT MediaSink::Initialize( void )
{
	...
	do {
		...
		if( NULL == m_pPresenter && m_dxVersion >= DX_11 ) {
		...
		if( NULL == m_pPresenter && m_dxVersion >= DX_9 ) {
