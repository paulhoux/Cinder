...
HRESULT Activate::CreateInstance( HWND hwnd, IMFActivate** ppActivate, DirectXVersion dxVersion )
{
	if( ppActivate == NULL ) {
		return E_POINTER;
	}

	Activate* pActivate = new Activate( dxVersion );
	if( pActivate == NULL ) {
...
HRESULT Activate::ActivateObject( __RPC__in REFIID riid, __RPC__deref_out_opt void** ppvObject )
{
	...
	do {
		if( m_pMediaSink == NULL ) {
			hr = MediaSink::CreateInstance( IID_PPV_ARGS( &m_pMediaSink ), m_dxVersion );
			BREAK_ON_FAIL( hr );
...
// ctor
Activate::Activate( DirectXVersion dxVersion )
	: m_lRefCount( 0 )
	, m_pMediaSink( NULL )
	, m_hwnd( NULL )
