...
HRESULT Scheduler::StartScheduler( IMFClock* pClock )
{
	...
	if( m_hWaitTimer == NULL ) {
		...
		CI_LOG_E( "Failed to create waitable timer. " << HRESULT_STRING( hr ) );
	}
...
//-----------------------------------------------------------------------------
HRESULT Scheduler::StopScheduler( void )
{
	if( m_hWaitTimer != NULL ) {
		CloseHandle( m_hWaitTimer );
		m_hWaitTimer = NULL;
...
HRESULT Scheduler::ProcessSample( IMFSample* pSample, LONG* plNextSleep )
{
	...
	if( m_pClock ) {
		...
		//CI_LOG_I( "Presentation: " << hnsPresentationTime << " System: " << hnsSystemTime << " Now: " << hnsTimeNow );
...
//-----------------------------------------------------------------------------
HRESULT Scheduler::StartProcessSample()
{
	HRESULT hr = S_OK;
	...
	do {
		...
		BREAK_ON_FAIL_MSG( hr, "Failed to process samples in queue." );

		if( INFINITE != lWait && 0 < lWait ) {
			//CI_LOG_I( "Wait until sample can be presented... " << lWait );
			...
			BREAK_ON_FAIL_MSG( hr, "Failed to set waitable timer." );

			// queue a waititem to wait for timer completion
			ScopedComPtr<IMFAsyncResult> pAsyncResult;
			hr = MFCreateAsyncResult( nullptr, &m_xOnTimer, nullptr, &pAsyncResult );
			BREAK_ON_FAIL_MSG( hr, "Failed to create async result." );

			hr = MFPutWaitingWorkItem( m_hWaitTimer, 0, pAsyncResult, &m_keyTimer );
			BREAK_ON_FAIL_MSG( hr, "Failed to put waiting work item." );
		}
	} while( FALSE );
...
//
//--------------------------------------------------------------------------
HRESULT Scheduler::OnTimer( __RPC__in_opt IMFAsyncResult *pResult )
{
	HRESULT hr = S_OK;
	...
	static LARGE_INTEGER last = now;
	CI_LOG_I( "OnTimer: " << ( now.QuadPart - last.QuadPart ) );
	last = now;*/
