...
HRESULT PresenterDX11::IsMediaTypeSupported( IMFMediaType* pMediaType )
{
	...
	do {
		...
		UINT32 uParX, uParY = 0;
		hr = MFGetAttributeSize( pMediaType, MF_MT_PIXEL_ASPECT_RATIO, &uParX, &uParY );
		CI_LOG_V( "Pixel Aspect Ratio: " << uParX << ":" << uParY << " Source Size: " << uimageWidthInPixels << "x" << uimageHeightInPixels << " Dest Size: " << uimageWidthInPixels << "x" << ( uimageHeightInPixels * uParX / uParY ) );
...
HRESULT PresenterDX11::CreateDXGIManagerAndDevice()
{
	...
	do {
		...
		BREAK_ON_FAIL_MSG( hr, "Failed to create D3D11 device." );

		if( NULL == m_pDXGIManager ) {
			UINT resetToken;
			hr = MFCreateDXGIDeviceManager( &resetToken, &m_pDXGIManager );
			BREAK_ON_FAIL_MSG( hr, "Failed to create DXGI device manager." );

			m_DeviceResetToken = resetToken;
		}

		hr = m_pDXGIManager->ResetDevice( m_pD3DDevice, m_DeviceResetToken );
		BREAK_ON_FAIL_MSG( hr, "Failed to reset D3D11 device." );
		...
		BREAK_ON_FAIL_MSG( hr, "Failed to query DXGI factory." );

		ScopedComPtr<IDXGIOutput> pDXGIOutput;
		hr = pAdapter->EnumOutputs( 0, &pDXGIOutput );
		BREAK_ON_FAIL_MSG( hr, "Failed to query DXGI output." );
		...
		if( m_useDCompVisual ) {
			...
			BREAK_ON_FAIL_MSG( hr, "Failed to create composition device." );
		}
		...
		desc.MiscFlags = 0;
		hr = m_pD3DDevice->CreateQuery( &desc, &m_pQuery );
		BREAK_ON_FAIL_MSG( hr, "Failed to create D3D11 event query." );
	} while( FALSE );
...
HRESULT PresenterDX11::FindBOBProcessorIndex( DWORD* pIndex )
{
	...
	do {
		...
		*pIndex = 0;
		...
		BREAK_ON_FAIL_MSG( hr, "Failed to query video processor caps." );

		for( DWORD i = 0; i < caps.RateConversionCapsCount; i++ ) {
			hr = m_pVideoProcessorEnum->GetVideoProcessorRateConversionCaps( i, &convCaps );
			BREAK_ON_FAIL_MSG( hr, "Failed to query video rate conversion caps." );
...
HRESULT PresenterDX11::ProcessFrameUsingD3D11( ID3D11Texture2D* pLeftTexture2D, ID3D11Texture2D* pRightTexture2D, UINT dwLeftViewIndex, UINT dwRightViewIndex, RECT rcDest, UINT32 unInterlaceMode, IMFSample** ppVideoOutFrame )
{
	...
	do {
		if( !m_pVideoDevice ) {
			hr = m_pD3DDevice->QueryInterface( __uuidof( ID3D11VideoDevice ), (void**)&m_pVideoDevice );
			BREAK_ON_FAIL_MSG( hr, "Failed to query video device." );
		}
		...
		BREAK_ON_FAIL_MSG( hr, "Failed to query video context." );
		...
		if( !m_pVideoProcessorEnum || !m_pVideoProcessor || m_imageWidthInPixels != surfaceDesc.Width || m_imageHeightInPixels != surfaceDesc.Height ) {
			...
			hr = m_pVideoDevice->CreateVideoProcessorEnumerator( &ContentDesc, &m_pVideoProcessorEnum );
			BREAK_ON_FAIL_MSG( hr, "Failed to create video processor enumerator." );
			...
			BREAK_ON_FAIL_MSG( hr, "Failed to find BOB processor index." );

			hr = m_pVideoDevice->CreateVideoProcessor( m_pVideoProcessorEnum, index, &m_pVideoProcessor );
			BREAK_ON_FAIL_MSG( hr, "Failed to create video processor." );
			...
			if( m_b3DVideo ) {
				...
				if( m_bFullScreenState ) {
					...
					BREAK_ON_FAIL_MSG( hr, "Failed to find closest matching model." );
				}
			}
		}
		...
		if( !m_pSwapChain1 || fDestRectChanged ) {
			...
			BREAK_ON_FAIL_MSG( hr, "Failed to update DXGI swap chain." );
		}
		...
		BREAK_ON_FAIL_MSG( hr, "Failed to query DXGI back buffer." );
		...
		if( ::IsWindowVisible( m_hwndVideo ) ) {
			...
			BREAK_ON_FAIL_MSG( hr, "Failed to create video processor output view." );
		}
		...
		BREAK_ON_FAIL_MSG( hr, "Failed to create video processor input view." );
		...
		/*if( m_b3DVideo ) {
			BREAK_ON_FAIL_MSG( E_NOTIMPL, "3D video is not supported." );
...
HRESULT PresenterDX11::BlitToShared( const D3D11_VIDEO_PROCESSOR_STREAM *pStream, UINT32 unInterlaceMode )
{
	...
	do {
		...
		BREAK_ON_FAIL_MSG( hr, "Failed to create shared texture." );
		...
		// Spin-wait until VideoProcessorBlt has finished.
		if( NULL != m_pQuery ) {
			m_pD3DImmediateContext->End( m_pQuery );
			while( S_OK != m_pD3DImmediateContext->GetData( m_pQuery, NULL, 0, 0 ) );
		}
