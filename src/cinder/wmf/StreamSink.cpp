...
//      2. Create an AsyncOperation object for the operation.
...
{
	// States:    Operations:
	//            SetType   Start     Restart   Pause     Stop      Sample    Marker    Flush
	/* NotSet */  TRUE,     FALSE,    FALSE,    FALSE,    FALSE,    FALSE,    FALSE,    TRUE,

	/* Ready */   TRUE,     TRUE,     TRUE,     TRUE,     TRUE,     FALSE,    TRUE,     TRUE,

	/* Start */   TRUE,     TRUE,     FALSE,    TRUE,     TRUE,     TRUE,     TRUE,     TRUE,

	/* Pause */   TRUE,     TRUE,     TRUE,     TRUE,     TRUE,     TRUE,     TRUE,     TRUE,

	/* Stop */    TRUE,     TRUE,     FALSE,    FALSE,    TRUE,     FALSE,    TRUE,     TRUE
...
HRESULT StreamSink::Flush( void )
{
	...
	if( SUCCEEDED( hr ) ) {
		...
		if( FAILED( hr ) )
			CI_LOG_E( "Failed to process samples from queue. " << HRESULT_STRING( hr ) );
	}

	if( SUCCEEDED( hr ) ) {
		// This call blocks until the scheduler threads discards all scheduled samples.
		if( FAILED( m_pScheduler->Flush() ) )
			CI_LOG_E( "Failed to flush scheduler. " );

		hr = m_pPresenter->Flush();
		if( FAILED( hr ) )
			CI_LOG_E( "Failed to flush presenter. " << HRESULT_STRING( hr ) );
	}
...
HRESULT StreamSink::GetMediaTypeHandler( __RPC__deref_out_opt IMFMediaTypeHandler** ppHandler )
{
	...
	if( SUCCEEDED( hr ) ) {
		...
		if( FAILED( hr ) )
			CI_LOG_E( "Failed to query media type handler. " << HRESULT_STRING( hr ) );
	}
...
HRESULT StreamSink::PlaceMarker( MFSTREAMSINK_MARKER_TYPE eMarkerType, __RPC__in const PROPVARIANT* pvarMarkerValue, __RPC__in const PROPVARIANT* pvarContextValue )
{
	...
	if( SUCCEEDED( hr ) ) {
		...
		if( FAILED( hr ) )
			CI_LOG_E( "Invalid operation. " << HRESULT_STRING( hr ) );
	}
	...
	if( SUCCEEDED( hr ) ) {
		...
		if( FAILED( hr ) )
			CI_LOG_E( "Failed to create marker. " << HRESULT_STRING( hr ) );
	}

	if( SUCCEEDED( hr ) ) {
		hr = m_SamplesToProcess.InsertBack( pMarker );
		if( FAILED( hr ) )
			CI_LOG_E( "Failed to insert marker. " << HRESULT_STRING( hr ) );
	}
	...
	if( SUCCEEDED( hr ) ) {
		if( m_state != State_Paused ) {
			...
			if( FAILED( hr ) )
				CI_LOG_E( "Failed to queue async operation. " << HRESULT_STRING( hr ) );
		}
	}
	SafeRelease( pMarker );
...
HRESULT StreamSink::ProcessSample( __RPC__in_opt IMFSample* pSample )
{
	...
	do {
		...
		if( !m_fPrerolling && !m_fWaitingForOnClockStart ) {
			...
			BREAK_ON_FAIL_MSG( hr, "Invalid operation." );
		}

		// Add the sample to the sample queue.
		hr = m_SamplesToProcess.InsertBack( pSample );
		BREAK_ON_FAIL_MSG( hr, "Failed to insert sample." );
		...
		if( m_state != State_Paused && m_state != State_Stopped ) {
			...
			BREAK_ON_FAIL_MSG( hr, "Failed to queue async operation." );
		}
	} while( FALSE );
...
HRESULT StreamSink::BeginGetEvent( IMFAsyncCallback* pCallback, IUnknown* punkState )
{
	...
	if( SUCCEEDED( hr ) ) {
		...
		if( FAILED( hr ) )
			CI_LOG_E( "Failed to begin get event. " << HRESULT_STRING( hr ) );
	}
...
HRESULT StreamSink::EndGetEvent( IMFAsyncResult* pResult, _Out_  IMFMediaEvent** ppEvent )
{
	...
	if( SUCCEEDED( hr ) ) {
		...
		if( FAILED( hr ) )
			CI_LOG_E( "Failed to end get event. " << HRESULT_STRING( hr ) );
	}
...
HRESULT StreamSink::GetEvent( DWORD dwFlags, __RPC__deref_out_opt IMFMediaEvent** ppEvent )
{
	...
	if( SUCCEEDED( hr ) ) {
		...
		if( FAILED( hr ) )
			CI_LOG_E( "Failed to get event. " << HRESULT_STRING( hr ) );
	}
...
HRESULT StreamSink::QueueEvent( MediaEventType met, __RPC__in REFGUID guidExtendedType, HRESULT hrStatus, __RPC__in_opt const PROPVARIANT* pvValue )
{
	...
	if( SUCCEEDED( hr ) ) {
		...
		if( FAILED( hr ) )
			CI_LOG_E( "Failed to get event param var. " << HRESULT_STRING( hr ) );
	}
...
HRESULT StreamSink::GetMediaTypeByIndex( DWORD dwIndex, _Outptr_ IMFMediaType** ppType )
{
	...
	do {
		...
		BREAK_ON_FAIL_MSG( hr, "Failed to create media type." );

		hr = pVideoMediaType->SetGUID( MF_MT_MAJOR_TYPE, MFMediaType_Video );
		BREAK_ON_FAIL_MSG( hr, "Failed to set media type." );

		hr = pVideoMediaType->SetGUID( MF_MT_SUBTYPE, subType );
		BREAK_ON_FAIL_MSG( hr, "Failed to set media subtype." );
...
HRESULT StreamSink::SetCurrentMediaType( IMFMediaType* pMediaType )
{
	...
	do {
		...
		BREAK_ON_FAIL_MSG( hr, "Failed to check if media type is supported." );
		...
		BREAK_ON_FAIL_MSG( hr, "Failed to get guid." );
		...
		if( m_unInterlaceMode == MFVideoInterlace_Progressive ) {
			...
			if( FAILED( hr ) )
				CI_LOG_E( "Failed to set required sample count. " << HRESULT_STRING( hr ) );
		}
		else {
		...
		if( SUCCEEDED( hr ) ) {
			...
			BREAK_ON_FAIL_MSG( hr, "Failed to set current media type." );
		}
...
HRESULT StreamSink::GetService( __RPC__in REFGUID guidService, __RPC__in REFIID riid, __RPC__deref_out_opt LPVOID* ppvObject )
{
	...
	if( SUCCEEDED( hr ) ) {
		...
		if( FAILED( hr ) )
			CI_LOG_E( "Failed to get service. " << HRESULT_STRING( hr ) );
	}
	return hr;
}
...
HRESULT StreamSink::PresentFrame( void )
{
	...
	if( SUCCEEDED( hr ) ) {
		...
		if( FAILED( hr ) )
			CI_LOG_E( "Failed to queue event. " << HRESULT_STRING( hr ) );
	}
...
HRESULT StreamSink::GetMaxRate( BOOL fThin, float* pflRate )
{
	...
	do {
		...
		hr = MFGetAttributeRatio( m_pCurrentType, MF_MT_FRAME_RATE, &unNumerator, &unDenominator );
...
HRESULT StreamSink::Initialize( IMFMediaSink* pParent, Presenter* pPresenter )
{
	...
	do {
		...
		BREAK_ON_FAIL_MSG( hr, "Failed to initialize attributes." );

		// Create the event queue helper.
		hr = MFCreateEventQueue( &m_pEventQueue );
		BREAK_ON_FAIL_MSG( hr, "Failed to create event queue." );

		// Allocate a new work queue for async operations.
		hr = MFAllocateWorkQueue( &m_WorkQueueId );
		BREAK_ON_FAIL_MSG( hr, "Failed to allocate work queue." );
...
HRESULT StreamSink::Pause( void )
{
	...
	do {
		...
		hr = QueueAsyncOperation( OpFlush );
		BREAK_ON_FAIL_MSG( hr, "Failed to queue async operation." );
...
HRESULT StreamSink::OnDispatchWorkItem( IMFAsyncResult* pAsyncResult )
{
	...
	do {
		...
		BREAK_ON_FAIL_MSG( hr, "Failed to get state." );

		// The state object is a CAsncOperation object.
		AsyncOperation* pOp = (AsyncOperation*)pState.get();
		...
		switch( op ) {
			...
			case OpRestart:
				...
				BREAK_ON_FAIL_MSG( hr, "Failed to queue event." );
				...
				hr = ProcessSamplesFromQueue( m_ConsumeData );
				BREAK_ON_FAIL_MSG( hr, "Failed to process samples from queue." );
				break;
			...
			case OpPlaceMarker:
				if( !( m_fWaitingForOnClockStart ) ) {
					...
					BREAK_ON_FAIL_MSG( hr, "Failed to dispatch process sample." );
				}

				// Scrubbing support.
				if( m_cOutstandingSampleRequests == 0 && m_pScheduler->GetClockRate() == 0.0f ) {
					hr = QueueEvent( MESessionScrubSampleComplete, GUID_NULL, hr, NULL );
					BREAK_ON_FAIL_MSG( hr, "Failed to queue event." );
				}
				break;
			...
			if( pMarker ) {
				...
				if( bConsumeData == ProcessFrames ) {
					...
					if( SUCCEEDED( hr ) ) {
						...
						CI_LOG_E( "Failed to process frame." );
...
HRESULT StreamSink::QueueAsyncOperation( StreamOperation op )
{
	...
	AsyncOperation* pOp = new AsyncOperation( op ); // Created with ref count = 1
	if( pOp == NULL ) {
		CI_LOG_E( "Out of memory." );
		hr = E_OUTOFMEMORY;
	}
	...
	if( SUCCEEDED( hr ) ) {
		...
		if( FAILED( hr ) )
			CI_LOG_E( "Failed to put work item." );
	}
...
/////////////////////////////////////////////////////////////////////////////////////////////
//
// AsyncOperation class. - Private class used by StreamSink class.
//
/////////////////////////////////////////////////////////////////////////////////////////////
...
StreamSink::AsyncOperation::~AsyncOperation( void )
{
}
