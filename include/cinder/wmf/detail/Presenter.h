...
#include "cinder/wmf/detail/MFAsyncCallback.h"
#include "cinder/wmf/detail/MonitorArray.h"
...
// If enabled, creates a DXVA2 decoder to do the video processing.
#define MF_USE_DXVA2_DECODER 0
#define MF_USE_COMPOSITION 0 // ( _WIN32_WINNT >= _WIN32_WINNT_WIN8 )  // Windows 8+ only.
#define MF_USE_DXVA_HD 0     // ( _WIN32_WINNT >= _WIN32_WINNT_WIN7 )  // Windows 7 desktop only.
...
DEFINE_GUID( GUID_ObjectHandle, 0x8a5ab099, 0xecce, 0x4c11, 0xa6, 0x5d, 0x23, 0xb4, 0x2f, 0x2b, 0xe0, 0xba );

// Forward-declare these classes, so we remain "compatible" with systems that have no support for them.
#ifdef __cplusplus
extern "C" {
#endif
...
class __declspec( uuid( "F57105CF-F608-4107-ADD5-B0CF01CBFD0D" ) ) Presenter : public IMFVideoDisplayControl, public IMFGetService {
  public:
	...
	virtual STDMETHODIMP_( UINT32 ) GetImageWidth() const { return m_imageWidthInPixels; }
	virtual STDMETHODIMP_( UINT32 ) GetImageHeight() const { return m_imageHeightInPixels; }
...
class __declspec( uuid( "011760FC-FF4C-4B8B-B0CE-823842C812E9" ) ) PresenterDX9 : public Presenter {
  ...
  private:
	...
	STDMETHODIMP TrackSample( IMFSample* pSample );
	HRESULT OnSampleFree( __RPC__in_opt IMFAsyncResult* pResult );

	STDMETHODIMP CreateDXVAHDDevice( IMFMediaType* pMediaType );
	STDMETHODIMP CreateVPDevice( IDXVAHD_Device* pDXVAHD, const DXVAHD_VPDEVCAPS& caps, IDXVAHD_VideoProcessor** ppDXVAVP );
	STDMETHODIMP CheckInputFormatSupport( IDXVAHD_Device* pDXVAHD, const DXVAHD_VPDEVCAPS& caps, D3DFORMAT d3dformat );
	STDMETHODIMP CheckOutputFormatSupport( IDXVAHD_Device* pDXVAHD, const DXVAHD_VPDEVCAPS& caps, D3DFORMAT d3dformat );
	...
	MFAsyncCallback<PresenterDX9> m_SampleFreeCB;
	...
	msw::CriticalSection m_critSecFrame;
	IDirect3DSurface9*   m_pCurrentFrame;
	IDirect3DSurface9*   m_pNextFrame;
